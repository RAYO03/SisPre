        ...
        <div class="bg-indigo-50 border border-indigo-100 rounded-xl p-4 mb-6">
            ...
            @if ($proximaVencida)
                <p class="text-sm text-indigo-600 mt-1">
                    ...
                    — pendiente:
                    <strong>${{ number_format($proximaVencida->cuota_total - $proximaVencida->monto_pagado, 2) }}</strong>
                </p>
            @endif
        </div>
            ...
            <form method="POST" action="{{ route('pagos.store', $prestamo) }}">
                ...
                <div class="space-y-4">
                    ...
                    @php
                        ...
                        $saldoPendiente = $cuotaActiva ? $cuotaActiva->cuota_total - $cuotaActiva->monto_pagado : 0;
                    @endphp

                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Monto del Pago *</label>
                        <input type="number" name="monto" step="0.01" min="0.01" max="{{ $saldoPendiente }}"
                            value="{{ old('monto', number_format($saldoPendiente, 2, '.', '')) }}"
